...
contract AVAXGods is ERC1155, Ownable, ERC1155Supply {
    ...
    /// @param inBattle boolean to indicate if a player is in battle
    ...
    struct Battle {
        ...
        address[2] players;
        /// @param players address array representing players in this battle
        uint8[2] moves;
        /// @param moves uint array representing players' move
        address winner;
    }
    ...
    function registerPlayer(string memory _name, string memory _gameTokenName) external {
        ...
        players.push(Player(msg.sender, _name, 10, 25, false)); // Adds player to players array. e Player is a struct
    ...
    function _createGameToken(string memory _name) internal returns (GameToken memory) {
        ...
        GameToken memory newGameToken = GameToken(_name, randId, randAttackStrength, randDefenseStrength);
